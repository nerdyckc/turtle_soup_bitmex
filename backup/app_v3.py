        ...
        if data_df is None:
            ...
            for trade in msg['data']:
                ...
                if trade_time <= data_df.index[-1]:
                    ...
                    logging.info('trade_entry execution time{}'.format(datetime.now() - start_time2))
                    ...
                    logging.info('mange_position execution time{}'.format(datetime.now() - start_time3))
    ...
    # elif msg['table'] == 'position' and len(msg['data'][0]) >= 38:
        ...
        if 'avgCostPrice' in msg['data'][0]:    # close entire position
            ...
            if symbol in POS_TABLE and abs(current_qty) < abs(POS_TABLE[symbol]['remain_qty']):
                ...
                logging.info('testing current_qty {}'.format(current_qty))
                ...
                logging.info('New Position: {} {} Entry: {} Stoploss: {}'.format(symbol, POS_TABLE[symbol]['remain_qty'], entry_price, stop_loss))
                if isOrderInTransit: isOrderInTransit = False
        if 'execQty' in msg['data'][0] and abs(current_qty) > 0:    # close partial position
            if symbol in POS_TABLE and abs(current_qty) < abs(POS_TABLE[symbol]['remain_qty']):
                ...
                logging.info('Position Update: {} {} Realised PnL: {real_pnl:.8f}'.format(symbol, POS_TABLE[symbol]['remain_qty'], real_pnl=real_pnl*0.00000001))
                if isOrderInTransit: isOrderInTransit = False
    logging.info('handle_data execution time{}'.format(datetime.now() - start_time1))
    ...
    while True:
        ...
        while status_code != 200:
            try:
                ...
                logging.info(e, '>>>>> 503 error, SLEEP, re-submit order!!!!!!!!!!!!')
                sleep(SLEEP_SECONDS)
                continue    # retry
        ...
        if symbol in POS_TABLE and orderID is None:     # populate POS_TABLE with orderID's
            ...
            if price is not None and POS_TABLE[symbol]['takeProfit_orderID'] is None:
                logging.info('fill in takeProfit orderID')
